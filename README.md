...
- 上传图片：`public/uploads/`
- 站点视觉素材：`public/assets/images/roxy/`
- 个人资料与歌单：`src/data/`
- 可选背景媒体：`public/media/`
- 内容后台：`/admin/`
...
唱片封面、LRC 歌词、Bilibili/Bangumi 标识、更新日志、工具页、相册、视觉素材库与友链。

相册照片在“苍蓝回廊设置 → 图库与相册 → 相册 → 相册照片”中直接新增、替换和排序。
“视觉素材库”统一列出当前站点图片、动图及用途，原始文件集中保存在
`public/assets/images/roxy/`；旧相册目录与 `urls.txt` 仍保留为兼容来源，不会静默删除。
