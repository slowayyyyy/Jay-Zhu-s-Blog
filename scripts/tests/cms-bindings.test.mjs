...
const azureFields = cms.collections.find((c) => c.name === "site_settings").files.find((f) => f.name === "azure").fields;
...
test("changed CMS title and short quotes reach the banner, with an empty-pool fallback", () => {
	...
	let actual = config("src/config/backgroundWallpaper.ts", content).backgroundWallpaper;
	assert.equal(actual.common.homeText.title, "测试横幅");
	assert.deepEqual(Array.from(actual.common.homeText.subtitle), ["第一条", "第二条"]);
	content.home.momentQuotes = [];
	content.visuals.backgroundVideo = "";
	actual = config("src/config/backgroundWallpaper.ts", content).backgroundWallpaper;
	assert.equal(actual.common.homeText.subtitle, "备用短句");
	assert.equal(actual.playerEnable, false);
	delete content.home.momentQuotes;
	assert.equal(config("src/config/backgroundWallpaper.ts", content).backgroundWallpaper.common.homeText.subtitle, "备用短句");
});
...
test("music configuration excludes empty sources without disturbing track/cover/lyrics/translation pairing", () => {
	...
	const tracks = config("src/config/musicConfig.ts", content).musicPlayerConfig.local.playlist;
	...
	content.playlist = [];
	assert.equal(config("src/config/musicConfig.ts", content).musicPlayerConfig.local.playlist.length, 0);
});
...
test("Umami public ID and script address use CMS settings; blank ID disables collection", () => {
	...
	let actual = config("src/config/analyticsConfig.ts", content).analyticsConfig.umamiAnalytics;
	...
	actual = config("src/config/analyticsConfig.ts", content).analyticsConfig.umamiAnalytics;
...
test("inactive opening and branch fields are hidden but still preserved in JSON", () => {
	...
	for (const name of ["hero", "home"]) {
		for (const field of azureFields.find((f) => f.name === name).fields.filter((f) => f.widget === "hidden")) {
...
test("all CMS file collections point to real files; about metadata is accepted and consumed", () => {
	for (const collection of cms.collections) {
		for (const file of collection.files || []) assert.ok(existsSync(new URL(file.file, root)), file.file);
	}
	const about = cms.collections.find((c) => c.name === "about_settings").files.find((f) => f.name === "about");
	...
	for (const field of about.fields.filter((f) => f.name !== "body")) {
		assert.ok(schema.includes(field.name + ": z.string().optional()"), field.name + " schema");
		assert.ok(page.includes("aboutData." + field.name), field.name + " rendering");
	}
});
...
test("bundled songs retain existing local cover and lyric file paths", () => {
	for (const track of data.playlist || []) {
		for (const path of [track.cover, track.lrc]) {
			if (path?.startsWith("/") && !path.startsWith("//") && !path.startsWith("/media/")) {
				assert.ok(existsSync(new URL("public" + path, root)), track.name + " " + path);
			}
		}
	}
});
